...
deploy() {
  ...
  DEPLOY_STATEMENT="deploy --deploymentorder $2 $DEPLOY_PROPS $1"
...
for deployment in $(find $DEPLOY_DIR -mindepth 1 -maxdepth 1 \( -name "*.war" -o -name "*.ear" -o -name "*.jar" -o -type d \) ! -name "*.rar" | sort); do
  deploy $deployment $counter;
